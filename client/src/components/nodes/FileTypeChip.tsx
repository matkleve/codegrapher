...
export function FileTypeChip({ filePath, className }: FileTypeChipProps) {
  ...
  return (
    <button
      ...
      className={cn(
        "file-type-chip hoverable nodrag inline-flex w-fit max-w-full shrink-0 cursor-pointer self-start items-center gap-1.5 rounded-full border px-2 py-0.5 text-left text-xs leading-none",
        chip.pillClass,
        className,
      )}
    ...
    >
      ...
      <span className="min-w-0 truncate font-mono leading-none">{filePath}</span>
    </button>
  );
}
