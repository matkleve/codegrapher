...
4. **Trace data flow** — which methods read/write a given property; where a local gets its value (binding).
...
| U3 | Tracer | follow a value to where it's read | I find the wrong branch | **hover a property** → fan-out to readers; **hover a local** → binding wire from initializer | V |
...
| U6 | Reviewer | keep that context while I read on | the answer doesn't vanish when I move | **click to pin**; Shift+click to accumulate; click-away / Esc to close | C F V |
...
| U12 | Explorer | tell an on-screen def from an off-screen one | I know if I must load it | in-graph → solid trace; **off-graph → dashed Load stub** | C F V |
| U13 | Explorer | pull an off-screen definition in | I grow the ego-graph one deliberate hop | **hover external token** → **TokenConnectionMenu** Load row(s) + optional **Load all · N** | C F V |
...
| U16 | Tracer | see which branches a decision can take | I don't mentally parse nested `switch`/`if` | **hover `switch`/`if` or discriminant** → control-flow fan-out | — |
| U17 | Architect | see class relationships without hovering | I map structure at a glance | load both classes → persistent **extends** / **implements** / DI wires | C |

---

## Design principles carried into the spec

- **Question, not fact.** Preview edges are summoned per keyword; structural edges are the named exception for permanent relationships.
- **Answer at the reader's zoom.** The edge lands as precisely as the target is
  revealed: class header → member row → exact line.
- **Direction encodes meaning.** Usage: **definition → usage**. Binding: initializer → name. Control flow: condition → branch.
- **Calm by default.** Dwell-gate plain hover; only Ctrl makes everything loud.
- **Locality is legible.** Solid = here, dashed stub = elsewhere; menu offers the load action (no floating pill).
