...
**Pane mood flicker:** leave+enter transitions run through the `traceMachine` statechart — if mood drops between chips, check `GRACE_EXPIRE` vs `POINTER_ENTER` order, not CSS. Source of truth: `useTraceSession` + `?trace-debug=1`.
