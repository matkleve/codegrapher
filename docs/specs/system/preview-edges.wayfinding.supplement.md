...
When **2+ wires** share the same source element and the same `connectionKind`, and their targets cluster within **104px vertical span** (`FAN_TARGET_Y_SPAN`), the overlay routes them as a **fan**:
...
| Usage / binding / transitive | **Cubic trunk** (same exit curve as solo wires) + cubic spurs |
