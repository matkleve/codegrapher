...
- Usage and transitive preview connections are **on-demand only** — never a persistent edge layer. **Binding** (initializer → local/param), **Typesetting** (sig-type → param def), and **Control flow** (`switch`/`if` → branch) are also on-demand. This applies to preview kinds in [connection-taxonomy.md](connection-taxonomy.md); it does not extend to structural kinds.
...
- Usage/transitive/binding/**typesetting**/control-flow preview rendering is **overlay-only** (`PreviewEdgeOverlay`), not React Flow edges. Structural wires use the same overlay's structural layer — see [connection-taxonomy.md](connection-taxonomy.md) Where It Lives.
